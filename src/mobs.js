...
export class MobSystem {
  ...
  makeMesh(type) {
    ...
    if (type === 'pig' || type === 'sheep') {
      ...
      this.part(group, 0.62, 0.58, 0.58, def.color, 0, 0.78, -0.82);
      for (const [x, z] of [[-0.35, -0.38], [0.35, -0.38], [-0.35, 0.38], [0.35, 0.38]]) {
        this.part(group, 0.2, 0.55, 0.2, type === 'pig' ? 0x9f6267 : 0x6c625c, x, 0.28, z);
      }
    } else if (type === 'spider') {
      ...
      for (const side of [-1, 1]) {
        for (let i = 0; i < 4; i++) {
          const leg = this.part(group, 0.72, 0.11, 0.11, 0x211a1a, side * 0.68, 0.32, -0.48 + i * 0.34);
          leg.rotation.z = side * (0.25 + Math.abs(i - 1.5) * 0.12);
        }
      }
    } else {
      const robe = type === 'villager';
      const bodyColor = robe ? 0x74472e : type === 'skeleton' ? 0xb8b6ad : def.color;
      this.part(group, robe ? 0.62 : 0.55, 0.85, 0.32, bodyColor, 0, 1.0, 0);
      this.part(group, 0.52, 0.52, 0.52, def.color, 0, 1.68, 0);
      if (robe) {
        this.part(group, 0.16, 0.18, 0.22, 0xb57952, 0, 1.62, -0.34);
        this.part(group, 0.7, 0.55, 0.42, 0x69402b, 0, 0.45, 0);
      } else {
        this.part(group, 0.2, 0.8, 0.2, bodyColor, -0.42, 1.0, 0);
        this.part(group, 0.2, 0.8, 0.2, bodyColor, 0.42, 1.0, 0);
  ...
  kill(mob) {
    if (mob.type === 'pig') this.onDrop?.(mob.pos.x, mob.pos.y + 0.5, mob.pos.z, I.RAW_MEAT, 2);
    else if (mob.type === 'sheep') this.onDrop?.(mob.pos.x, mob.pos.y + 0.5, mob.pos.z, I.RAW_MEAT, 1);
  ...
  update(dt, player, { night = false, spawning = true } = {}) {
    ...
    for (let i = this.list.length - 1; i >= 0; i--) {
      ...
      if (mob.kind === 'hostile' && distance < 24) {
        if (mob.type === 'skeleton') {
          ...
          if (mob.type === 'creeper') {
            ...
            this.onPlayerDamage?.(
              ...
              mob.type === 'spider' ? 'Slain by a spider' : 'Slain by a zombie',
            );
